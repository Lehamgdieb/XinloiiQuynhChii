...
local function isDoingHazeQuest()
    ...
    pcall(function()
        ...
        if type(inv) == "table" then
            for _, item in pairs(inv) do
                if item.Name == "Alucard Fragment" then frags = item.Count or 0 break end
            end
        end
    end)
    ...
    if questContainer then
        ...
        if titleFrame then
            ...
            if textLabel and textLabel.Text then
                if textLabel.Text:find("Haze") then return true end
            end
        end
    end
    return false
...
local function verifyKeyAPI(key)
    ...
    local success, response = pcall(function() return request_func({ Url = API_VERIFY .. key, Method = "GET" }) end)
...
local function requestKeyUI(callback)
    ...
    Status.Text = "Key dung duoc 420 phut"
...
if AutoFindVIPBoss or AutoHopBoss then
    task.spawn(function()
        ...
        ensureVIPKey(function(ok)
            if ok then hopAllowed = true; print("[Banana VIP] Key hop le, mo khoa Hop Server!") end
        end)
    end)
end
...
task.spawn(function()
    ...
    local function IsMirageIsland()
        ...
        return map and map:FindFirstChild("MysticIsland") and "Co" or "Khong"
    end

    local function CheckMoon()
        local sky = Lighting:FindFirstChild("Sky")
        if not sky then return "Khong ro" end
    ...
    local function CheckElite()
        ...
        scan(workspace)
        return #found > 0 and table.concat(found, ", ") or "Khong"
    end

    task.spawn(function()
        while wait(Sec) do
            pcall(function()
                if Boud and plr.Character and not plr.Character:FindFirstChild("HasBuso") then CommF_("Buso") end
            end)
        end
    end)
    ...
    local function EquipMeleeIfNeeded()
        ...
        if cur and (cur:FindFirstChild("Handle") or cur.ToolTip:lower():find("sword") or cur.ToolTip:lower():find("melee")) then return true end
        local melee = nil
        for _, t in ipairs(plr.Backpack:GetChildren()) do
            if t:IsA("Tool") and (t:FindFirstChild("Handle") or t.ToolTip:lower():find("sword") or t.ToolTip:lower():find("melee")) then melee = t; break end
        end
        if melee then hum:EquipTool(melee); return true end
        for _, t in ipairs(plr.Backpack:GetChildren()) do if t:IsA("Tool") then hum:EquipTool(t); return true end end
        return false
    end
            ...
            if folder then
                for _, v in pairs(folder:GetChildren()) do
                    if v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 and v:FindFirstChild("HumanoidRootPart") then
                        if (v.HumanoidRootPart.Position - pos).Magnitude <= AttackRange then table.insert(targs, v) end
                    end
                end
            end
        end
    ...
    task.spawn(function() while true do if AutoAttack then pcall(Attack) end task.wait(AttackSpeed) end end)

    local isHopping = false
    local function performHop()
        if _G.IsDoingAutoCDK then HopLabel.Text = "HOP: Bi chan (dang Auto CDK)"; return end
        if isHopping then return end
        isHopping = true
        if needIndra or needDough then
            HopLabel.Text = "HOP: Dang quet DB VIP..."
            local vip = {}
            ...
            if #vip > 0 then
                ...
                HopLabel.Text = "HOP: Dang vao phong VIP..."
        ...
        HopLabel.Text = "HOP: Dang do Map Random..."
        task.spawn(function()
    ...
    local function HopToVIPBoss()
        ...
        if hopAllowed then performHop() else HopLabel.Text = "HOP: Vui long nhap Key VIP!" end
    end
    ...
    task.spawn(function()
        while true do
            pcall(function()
                ...
                local function scan(folder)
                    ...
                    for _, v in pairs(folder:GetChildren()) do
                        if table.find(BossList, v.Name) and not seen[v.Name] and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 then
                            seen[v.Name] = true; table.insert(foundBosses, v.Name)
                        end
                    end
                end
                scan(workspace:FindFirstChild("Enemies"))
                ...
                local payload = { boss_name = #foundBosses > 0 and table.concat(foundBosses, ", ") or "None", elite = CheckElite(), place_id = tostring(game.PlaceId), job_id = tostring(game.JobId), players = #Players:GetPlayers() .. "/" .. Players.MaxPlayers, mirage = IsMirageIsland(), full_moon = GetCurrentSea() == 3 and CheckMoon() or "Khong" }
                pcall(function() request_func({ Url = API_SAVE, Method = "POST", Headers = { ["Content-Type"] = "application/json" }, Body = HttpService:JSONEncode(payload) }) end)
                if (AutoFindVIPBoss or AutoHopBoss) and game.PlaceId == 100117331123089 then
                    local hasVip = false
                    for _, b in pairs(foundBosses) do if (b == "Dough King" and needDough) or (b == "rip_indra True Form" and needIndra) then hasVip = true break end end
                    if not hasVip and not isHopping and not _G.IsDoingAutoCDK then HopToVIPBoss(); lastMoveTime = tick() end
                end
                if plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
                    local pos = plr.Character.HumanoidRootPart.Position
                    if (pos - lastPos).Magnitude > 2 then lastPos = pos; lastMoveTime = tick() end
                else lastMoveTime = tick() end
                if tick() - lastMoveTime > AFK_TIMEOUT and not isHopping and (AutoFindVIPBoss or AutoHopBoss) and not _G.IsDoingAutoCDK then HopToVIPBoss(); lastMoveTime = tick() end
            end)
            task.wait(1)
        end
    end)
...
if cdkCfg.Enabled then
    ...
    local function EquipSword(itemName)
        local char = plr.Character
        if not char or char.Humanoid.Health <= 0 then return end
        if _G.IsTakingDamage then char.Humanoid:UnequipTools(); return end
    ...
    local function SmartMove(targetCFrame) Tween2(targetCFrame) end
        ...
        while task.wait() do
            if _G.Auto_DualKatana and _G.AutoFarm_Bone then
                pcall(function()
                    local target = nil
                    for _, v in pairs(workspace.Enemies:GetChildren()) do
                        if table.find(BoneMobs, v.Name) and v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 then target = v; break end
                    end
                    if target then
            ...
            if _G.Auto_DualKatana then
                if _G.AutoFarm_Bone then
                    ...
                    pcall(function()
                        ...
                        if not _G.IsTakingDamage then EquipSword(_G.CurrentSword) end
                        Auto_Quest_Yama_1, Auto_Quest_Yama_2, Auto_Quest_Yama_3 = false, false, false
                        Auto_Quest_Tushita_1, Auto_Quest_Tushita_2, Auto_Quest_Tushita_3 = false, false, false
                        if frags == 6 then
                            local boss = workspace.Enemies:FindFirstChild("Cursed Skeleton Boss")
                            if boss and boss.Humanoid.Health > 0 then SmartMove(boss.HumanoidRootPart.CFrame * Pos); AttackNoCoolDown() end
                        elseif frags == 5 then Auto_Quest_Yama_3 = true
                        elseif frags == 4 then Auto_Quest_Yama_2 = true
                        elseif frags == 3 then Auto_Quest_Yama_1 = true
                        elseif frags == 2 then Auto_Quest_Tushita_3 = true
                        elseif frags == 1 then Auto_Quest_Tushita_2 = true
                        elseif frags == 0 then Auto_Quest_Tushita_1 = true
                        end
                    end)
                end
            end
        ...
        while task.wait() do
            if Auto_Quest_Tushita_1 and not _G.AutoFarm_Bone then
                pcall(function()
                    ...
                    if target then
                        print("🎯 Tushita Q1: Bay toi Boat Dealer " .. _G.DealerStep .. "/3")
                        Tween2(target)
                        if (plr.Character.HumanoidRootPart.Position - target.Position).Magnitude <= 15 then
                            ...
                            if dealer then
                                ...
                                if _G.DealerStep > 3 then
                                    ...
                                    print("✅ Tushita Q1 hoan thanh!")
                                end
                            else
                                print("⚠️ Khong tim thay Luxury Boat Dealer, thu lai...")
            ...
            if Auto_Quest_Tushita_2 and not _G.AutoFarm_Bone then
                pcall(function()
                    if (CFrame.new(-5539, 313, -2972).Position - plr.Character.HumanoidRootPart.Position).Magnitude > 500 then
                        ...
                        else AutoHop(API_PIRATE_RAID, "Tim Pirate Raid") end
                    end
                end)
            end
        end
                    ...
                    if heav and (plr.Character.HumanoidRootPart.Position - heav.Spawn.Position).Magnitude < 3000 then
                        ...
                        if not foundMob then
                            ...
                            if allLit then
                                ...
                                if exitP then plr.Character.HumanoidRootPart.CFrame = exitP.CFrame end
                            end
                        end
                    else
                        local cq = nil
                        ...
                        if cq then 
                            ...
                            if (plr.Character.HumanoidRootPart.Position - Vector3.new(-709, 381, -11011)).Magnitude < 200 then
                                ...
                                if tick() - _G.CQDeadTimer > 7 then AutoHop(API_CAKE_QUEEN, "Tim Cake Queen") end
                            end
                        end
                    end
                end)
                ...
                pcall(function()
                    ...
                    if not foundHaze then
                        local HazeIslands = {
                            ...
                            CFrame.new(5319.0, 1005.4, 360.8), CFrame.new(-16270.0, 25.2, 1373.8)
                        }
                        if _G.NeedResetFromSubmerged then
                            print("Yama Q2: Dang tu sat de thoat khoi Tau Ngam...")
                        ...
                        elseif _G.HzIdx == 14 then
                            print("Yama Q2: Tuong tac NPC SubmarineWorkerSpeak...")
                            ...
                            print("Yama Q2: Tuan tra dao " .. _G.HzIdx .. "/14")
                            Tween2(HazeIslands[_G.HzIdx])
                ...
                pcall(function()
                    ...
                    if hell and (plr.Character.HumanoidRootPart.Position - hell.Spawn.Position).Magnitude < 3000 then
                        ...
                        if reaper and reaper.Humanoid.Health > 0 then
                            print("Yama Q3: Hung don tu Soul Reaper...")
                            ...
                            if plr.Backpack:FindFirstChild("Hallow Essence") or plr.Character:FindFirstChild("Hallow Essence") then
                                print("Yama Q3: Dang dem Hallow Essence di trieu hoi Boss...")
                                ...
                                AutoHop(API_SOUL_REAPER, "Tim Soul Reaper")
                                local bones = CommF_:InvokeServer("Bones", "Check") or 0
                                if bones >= 50 then
                                    print("Yama Q3: Du 50 Xuong, Dang bay di Random...")
                                    ...
                                    print("Yama Q3: Dang Farm Xuong kiem Essence ("..bones.."/50)...")
                                    local target = nil
                                    for _, v in pairs(workspace.Enemies:GetChildren()) do
                ...
                if hasY and hasT and not hasC then
                    if raceStatus ~= "V3" and raceStatus ~= "V4" then
                        ...
                        if not _G.Auto_DualKatana then
                            ...
                            print("[Auto CDK] Bat dau Auto CDK!")
                        end
                    end
                    return
                end
...
-- =====================================================================
-- RACE MASTER (HOÀN CHỈNH - TỰ ĐỘNG VỀ SEA 2 VÀ QUAY LẠI SEA 3)
-- =====================================================================
task.spawn(function()
                ...
                while task.wait(1.5) do
                    ...
                    pcall(function()
                        ...
                        if currentStatus == "V3" or currentStatus == "V4" then
                            ...
                            if isSea2 then
                                print("[Race Master] Da hoan thanh V3/V4, dang quay lai Sea 3...")
                                pcall(function() CommF_("TravelZou") end)
                                task.wait(8)
                                if hasYama() and hasTushita() and not hasCDK() then
                                    _G.Auto_DualKatana = true
                                    _G.IsDoingAutoCDK = true
                                    print("[Race Master] Da bat Auto CDK sau khi hoan thanh V3!")
                                end
                            end
                            return
                        end
                        ...
                        if not isSea2 then
                            if _G.AutoRaceV2 or _G.AutoRaceV3 then
                                print("[Race Master] Chua o Sea 2, dang yeu cau dich chuyen...")
                                ...
                                if waited > 30 then
                                    print("[Race Master] Qua thoi gian cho ve Sea 2, thu lai sau.")
                                    return
                                end
                                task.wait(3)
                                print("[Race Master] Da den Sea 2, bat dau tien trinh Race.")
                            else
                                return
                            end
                        end
